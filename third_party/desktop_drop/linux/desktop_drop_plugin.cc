...
void desktop_drop_plugin_register_with_registrar(FlPluginRegistrar *registrar) {
  ...
  // hyprview patch: GTK takes the first of these targets that the source
  // offers. Prefer plain URIs, then STRING, and use the portal transfer
  // key only as a last resort: file managers such as Dolphin offer both,
  // and resolving their key can fail ("Invalid transfer"), which dropped
  // the files. Upstream registers the portal target first.
  ...
  gtk_target_list_add(targets, gdk_atom_intern_static_string("STRING"),
                      GTK_TARGET_OTHER_APP, 0);
  gtk_target_list_add(
      targets,
      gdk_atom_intern_static_string("application/vnd.portal.filetransfer"),
      GTK_TARGET_OTHER_APP, 0);
  gtk_drag_dest_set_target_list(GTK_WIDGET(fl_view), targets);
