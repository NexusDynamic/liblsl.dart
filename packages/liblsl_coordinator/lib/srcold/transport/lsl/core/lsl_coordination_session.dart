...
import 'lsl_discovery_manager.dart';
...
class LSLCoordinationSession implements CoordinationSession, ManagedResource {
  ...
  final LSLCoordinationConfig _lslConfig;

  final LSLConnectionManager _connectionManager;
  final LSLDiscoveryManager _discoveryManager;
  ...
  LSLCoordinationSession({
    ...
    LSLCoordinationConfig? lslConfig,
    ...
    ConnectionConfig? connectionConfig,
       ...
       _discoveryManager = LSLDiscoveryManager(
         managerId: '${sessionId}_discovery',
         nodeId: nodeId,
         config: LSLDiscoveryConfig(
           nodeDiscoveryTimeout: lslConfig?.nodeDiscoveryTimeout,
           nodeDiscoveryInterval:
               lslConfig?.nodeDiscoveryInterval ?? const Duration(seconds: 5),
           resolverForgetAfter: lslConfig?.resolverConfig.forgetAfter ?? 5.0,
           maxStreamsPerResolver:
               lslConfig?.resolverConfig.maxStreamsPerResolver ?? 50,
         ),
       ),
    ...
    if (heartbeatInterval != null) {
      ...
      _heartbeatInterval = _lslConfig.heartbeatInterval;
    }
  ...
  Future<void> join() async {
    ...
    try {
      ...
      logger.info('DEBUG: Initializing discovery manager');
      await _discoveryManager.initialize();
      await _discoveryManager.start();
      logger.info('DEBUG: Discovery manager started');
      ...
      // Establish data stream connections now that session is active
      await establishDataStreamConnections();
    } catch (e) {
  ...
  Future<List<NetworkInfo>> _discoverMatchingNetworks() async {
    ...
    // Use discovery manager to find coordination streams
    final coordinationPredicate = _lslConfig.resolverConfig
        .coordinationPredicate('coordination');
    final streams = await _discoveryManager.discoverByPredicate(
      coordinationPredicate,
      timeout: _lslConfig.nodeDiscoveryTimeout,
    );

    logger.info(
      'DEBUG: Discovery manager found ${streams.length} coordination streams',
    );
  ...
  Future<void> _setupPeerConnections() async {
    try {
      ...
      // Get current peer streams (non-blocking from continuous resolver)
      final peerStreams = await _peerResolver!.resolve(waitTime: 0.0);
  ...
  Future<void> _setupHierarchicalConnections() async {
    try {
      if (role == NodeRole.server) {
        ...
        final clientStreams = await _clientResolver!.resolve(waitTime: 0.0);
  ...
  Future<void> _setupHybridConnections() async {
    try {
      ...
      if (role == NodeRole.leader) {
        ...
        final leaderStreams = await _peerResolver!.resolve(waitTime: 0.0);
        logger.info(
  ...
  Future<void> _discoverNetworkNodes() async {
    // Look for other nodes in our specific session using discovery manager
    final nodePredicate = _buildNodeDiscoveryPredicate();

    // Use continuous discovery instead of blocking resolver
    final streams = await _discoveryManager.discoverByPredicate(nodePredicate);
  ...
  Future<void> _checkForCoordinatorAndElect() async {
    try {
      // Look for existing coordinators (servers/leaders) using discovery manager
      final coordinatorPredicate =
      ...
      // Use continuous discovery for coordinator election
      final coordinatorStreams = await _discoveryManager.discoverByPredicate(
      ...
      if (coordinatorStreams.isEmpty) {
        ...
        if (topology == NetworkTopology.hybrid) {
          final electionResult = await _electionProtocol.triggerElection(
            candidateId: _nodeId,
            knownNodes: _networkState.nodes,
          );

          // Handle election result - promote to coordinator if this node won
          await _handleElectionResult(electionResult);
        }
      } else {
  ...
  /// Handle election result and promote node if it won
  Future<void> _handleElectionResult(ElectionResult result) async {
    try {
      if (result.hasWinner && result.winner!.nodeId == _nodeId) {
        logger.info(
          'This node won the election! Promoting to coordinator role',
        );

        // Update network state to reflect new coordinator role
        await _promoteToCoordinator();

        // Create coordination outlet to announce coordinator role
        await _createCoordinatorOutlet();

        _eventController.add(SessionStarted(sessionId));
      } else {
        logger.info(
          'Election completed - winner: ${result.winner?.nodeId ?? "none"}',
        );

        // Update network state with the election winner
        if (result.winner != null) {
          // Note: NetworkState.updateNodeRole method needs to be implemented
          logger.info(
            'Election winner: ${result.winner!.nodeId} - should update network state',
          );
        }
      }

      // Notify election protocol of the result
      await _electionProtocol.handleElectionResult(result);
    } catch (e) {
      logger.severe('Error handling election result: $e');
    }
  }

  /// Promote this node to coordinator role
  Future<void> _promoteToCoordinator() async {
    logger.info('Promoting node $_nodeId to coordinator');

    // Update internal state
    _updateState(SessionState.active);

    // Update network state role (method needs to be implemented)
    logger.info('Node $_nodeId promoted to coordinator role');

    // Start coordinator responsibilities
    await _startCoordinatorDuties();
  }

  /// Create outlet to announce coordinator role to the network
  Future<void> _createCoordinatorOutlet() async {
    try {
      // Create coordination stream config for coordinator role
      final coordinatorConfig = _buildCoordinatorStreamConfig();

      // Create outlet through connection manager
      await _connectionManager.createOutlet(
        outletId: '${_nodeId}_coordinator',
        config: coordinatorConfig,
      );

      logger.info('Created coordinator outlet for node $_nodeId');
    } catch (e) {
      logger.severe('Failed to create coordinator outlet: $e');
      rethrow;
    }
  }

  /// Start coordinator-specific duties
  Future<void> _startCoordinatorDuties() async {
    logger.info('Starting coordinator duties for node $_nodeId');

    // Start monitoring other nodes more actively
    _startEnhancedNodeDiscovery();

    // Begin coordination message processing
    // This would include processing requests from clients, managing data streams, etc.
  }

  /// Enhanced node discovery for coordinators
  void _startEnhancedNodeDiscovery() {
    // Coordinators need more frequent discovery to manage the network
    _nodeDiscoveryTimer?.cancel();
    _nodeDiscoveryTimer = Timer.periodic(_lslConfig.nodeDiscoveryInterval, (
      _,
    ) async {
      await _discoverNetworkNodes();
    });
  ...
  /// Build stream config for coordinator announcement
  LSLStreamConfig _buildCoordinatorStreamConfig() {
    return LSLStreamConfig(
      id: '${sessionId}_coordinator',
      maxSampleRate: 10.0, // Low rate for coordination messages
      pollingFrequency: 10.0,
      channelCount: 1,
      channelFormat: CoordinatorLSLChannelFormat.string,
      protocol:
          const ProducerOnlyProtocol(), // Coordinator produces coordination messages
      sourceId: '${_nodeId}_coord',
      metadata: {
  ...
  // === COORDINATOR-DIRECTED DATA STREAM METHODS ===

  /// Direct a data stream to connect to specific discovered streams
  /// This replaces the old self-discovery pattern
  Future<void> connectDataStreamTo(
    String streamId,
    List<LSLStreamInfo> targetStreams,
  ) async {
    final dataStream = _dataStreams[streamId];
    if (dataStream == null) {
      throw CoordinationSessionException('Data stream not found: $streamId');
    }

    logger.info(
      'Coordinator directing stream $streamId to connect to ${targetStreams.length} targets',
    );

    try {
      for (final targetStream in targetStreams) {
        await dataStream.connectToStream(targetStream);
      }
      logger.info(
        'Successfully connected data stream $streamId to all targets',
      );
    } catch (e) {
      logger.severe('Failed to connect data stream $streamId: $e');
      rethrow;
    }
  }

  /// Direct a data stream to discover and connect to streams matching a role
  Future<void> connectDataStreamToRole(
    String streamId,
    discovery.NodeRole targetRole, {
    ...
    logger.info(
      'Coordinator discovering streams for role $targetRole to connect stream $streamId',
    );

    try {
      // Use discovery manager to find streams with the target role
      final discoveredStreams = await _discoveryManager.discoverByRole(
        targetRole,
        metadata: metadataFilters,
        timeout: Duration(seconds: 30), // Use a reasonable timeout
      );

      if (discoveredStreams.isEmpty) {
        logger.warning('No streams found for role $targetRole');
        return;
      }

      // Connect to all discovered streams
      await connectDataStreamTo(streamId, discoveredStreams);
    } catch (e) {
      logger.severe(
        'Failed to connect data stream $streamId to role $targetRole: $e',
      );
      rethrow;
    }
  }

  /// Direct a data stream to disconnect from specific addresses
  Future<void> disconnectDataStreamFrom(
    String streamId,
    List<String> streamAddresses,
  ) async {
    final dataStream = _dataStreams[streamId];
    if (dataStream == null) {
      throw CoordinationSessionException('Data stream not found: $streamId');
    ...
    logger.info(
      'Coordinator directing stream $streamId to disconnect from ${streamAddresses.length} addresses',
    );

    try {
      for (final address in streamAddresses) {
        await dataStream.disconnectFromStream(address);
      }
      logger.info(
        'Successfully disconnected data stream $streamId from all targets',
      );
    } catch (e) {
      logger.severe('Failed to disconnect data stream $streamId: $e');
      rethrow;
    }
  }

  /// Get connection status for all managed data streams
  Map<String, List<String>> getDataStreamConnections() {
    final connections = <String, List<String>>{};
    for (final entry in _dataStreams.entries) {
      connections[entry.key] = entry.value.getConnectedStreams();
    }
    return connections;
  }

  /// Automatically establish data stream connections based on network topology
  /// This is the main coordinator logic for data stream management
  Future<void> establishDataStreamConnections() async {
    if (_state != SessionState.active) {
      logger.warning(
        'Cannot establish data stream connections - session not active',
      );
      return;
    }

    logger.info(
      'Coordinator establishing data stream connections based on network topology',
    );

    try {
      // For each data stream, determine what it should connect to based on its protocol
      for (final entry in _dataStreams.entries) {
        final streamId = entry.key;
        final dataStream = entry.value;
        final protocol = dataStream.config.protocol;

        if (protocol.isConsumer || protocol.isRelay) {
          // Consumer streams should connect to producer streams of the same type
          logger.info('Establishing consumer connections for stream $streamId');

          // Use discovery manager to find compatible producer streams
          final producerStreams = await _discoveryManager.discoverByPredicate(
            "contains(desc/stream_purpose, 'data_production')",
            timeout: const Duration(seconds: 5),
          );

          if (producerStreams.isNotEmpty) {
            await connectDataStreamTo(streamId, producerStreams);
          }
        }
      }

      logger.info('Data stream connections established successfully');
    } catch (e) {
      logger.severe('Failed to establish data stream connections: $e');
      rethrow;
    }
  }
}
