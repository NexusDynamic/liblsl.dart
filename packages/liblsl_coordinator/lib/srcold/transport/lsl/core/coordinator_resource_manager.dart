...
/// - Isolate controllers
class CoordinatorResourceManager implements ConnectionManager {
  @override
  ...
  Future<void> initialize() async {
    if (_isActive) {
      logger.warning(
        'CoordinatorResourceManager $managerId already initialized',
      );
      return;
    }
    ...
    for (final entry in _connectionManagers.entries) {
      try {
        ...
        _eventController.add(
          ResourceError(entry.key, 'Disposal error: $e', cause: e),
        );
      }
    }
  ...
  NetworkStats getUsageStats() {
    ...
    return NetworkStats(
      ...
      customMetrics: {
        'sessions':
            _resources.values.whereType<LSLCoordinationSession>().length,
    ...
    connectionManager.connectionEvents.listen((event) {
      _eventController.add(
        ResourceStateChanged(
          ...
          managerId: connectionManager.managerId,
        ),
      );
    });
  }
  ...
  Future<void> _performHealthCheck() async {
    ...
    for (final entry in _connectionManagers.entries) {
      healthCheckFutures.add(
        _checkConnectionManagerHealth(entry.key, entry.value),
      );
    }
  ...
  Future<void> _checkResourceHealth(
    String resourceId,
    ...
    try {
      ...
      if (!isHealthy) {
        ...
        _eventController.add(ResourceError(resourceId, 'Health check failed'));
      }
    } catch (e) {
      logger.warning('Error checking health of resource $resourceId: $e');
      _eventController.add(
        ResourceError(resourceId, 'Health check error: $e', cause: e),
      );
    }
  }

  Future<void> _checkConnectionManagerHealth(
    String managerId,
    LSLConnectionManager manager,
  ) async {
    try {
      ...
      logger.fine(
        'Connection manager $managerId: ${stats.totalResources} resources',
      );
    } catch (e) {
      logger.warning(
        'Error checking health of connection manager $managerId: $e',
      );
      _eventController.add(
        ResourceError(
          managerId,
  ...
  Future<void> _disposeResource(
    ...
    try {
      ...
      _eventController.add(
        ResourceError(resourceId, 'Disposal error: $e', cause: e),
      );
    }
  }
}
