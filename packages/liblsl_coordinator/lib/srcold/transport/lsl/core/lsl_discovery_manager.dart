...
class LSLDiscoveryManager implements ConnectionManager {
  @override
  ...
  final Map<String, LSLStreamResolverContinuousByPredicate>
  _predicateResolvers = {};
  ...
  String nodeRolePredicate(NodeRole role, [Map<String, String>? metadata]) {
    switch (role) {
      case NodeRole.discovering:
        return 'type="coordination_discovering"';
      case NodeRole.leader:
        return 'type="coordination_leader"';
      case NodeRole.server:
        return 'type="coordination_server"';
      case NodeRole.client:
        return 'type="coordination_client"';
  ...
  Future<List<LSLStreamInfo>> discoverByRole(
    ...
    final predicate = nodeRolePredicate(role, metadata);
  ...
  Future<List<LSLStreamInfo>> discoverByPredicate(
    ...
    try {
      ...
      _discoveryEventController.addEvent(
        LSLDiscoveryError(
          managerId,
          'Discovery failed for predicate $predicate: $e',
        ),
      );
      rethrow;
    }
  ...
  String startContinuousDiscoveryByPredicate(String predicate) {
    ...
    try {
      ...
      _discoveryEventController.addEvent(
        LSLDiscoveryError(
          managerId,
          'Failed to start continuous discovery: $e',
        ),
      );
      rethrow;
    }
  ...
  void stopContinuousDiscovery(String resolverId) {
    try {
      ...
      _discoveryEventController.addEvent(
        LSLDiscoveryError(
          managerId,
          'Error stopping continuous discovery $resolverId: $e',
        ),
      );
    }
  }
  ...
  Future<List<LSLStreamInfo>> getDiscoveredStreams(String resolverId) async {
    ...
    try {
      ...
      _discoveryEventController.addEvent(
        LSLDiscoveryError(
          managerId,
          'Error getting streams from $resolverId: $e',
        ),
      );
      rethrow;
    }
      ...
      for (final entry in _predicateResolvers.entries) {
        try {
          ...
          _discoveryEventController.addEvent(
            LSLDiscoveryError(
              managerId,
              'Periodic discovery error for ${entry.key}: $e',
            ),
          );
        }
      }
  ...
  List<String> get resolverIds {
    return [..._predicateResolvers.keys, ..._activeResolvers.keys];
  }
}
...
class LSLDiscoveryManagerStarted extends LSLDiscoveryEvent {
  LSLDiscoveryManagerStarted(String managerId)
    : super(managerId, DateTime.now());
}

class LSLDiscoveryManagerStopped extends LSLDiscoveryEvent {
  LSLDiscoveryManagerStopped(String managerId)
    : super(managerId, DateTime.now());
}
