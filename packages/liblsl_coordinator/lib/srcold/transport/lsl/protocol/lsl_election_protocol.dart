...
class LSLElectionProtocol extends ElectionProtocol {
  ...
  /// Trigger an election with this node as a candidate
  /// This is the method called by the coordination session when no coordinators are found
  @override
  Future<ElectionResult> triggerElection({
    ...
    logger.info('Triggering election for candidate $candidateId');
    
    // Create candidate list - if no known nodes, this node becomes coordinator by default
    final candidates = <NetworkNode>[];
    
    if (knownNodes != null && knownNodes.isNotEmpty) {
      candidates.addAll(knownNodes);
    }
    
    // Add this node as a candidate if not already included
    if (!candidates.any((n) => n.nodeId == candidateId)) {
      candidates.add(NetworkNode(
        nodeId: candidateId,
        nodeName: candidateId,
        role: NodeRole.discovering,
        lastSeen: DateTime.now(),
        metadata: {
          'capability_score': 100, // Default capability score
    ...
    return await startElection(candidates);
  }
}
