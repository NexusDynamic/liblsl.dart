...
abstract class ElectionProtocol extends Protocol {
  ...
  /// Trigger an election with the given candidate
...
/// Election events
sealed class ElectionEvent extends TimestampedEvent {
  final String electionId;

  const ElectionEvent(this.electionId, DateTime timestamp)
    : super(eventId: 'election_event_$electionId', timestamp: timestamp);
}

class ElectionStarted extends ElectionEvent {
  final List<NetworkNode> candidates;

  ElectionStarted(String electionId, this.candidates)
    : super(electionId, DateTime.now());
}

class ElectionCompleted extends ElectionEvent {
  final ElectionResult result;

  ElectionCompleted(String electionId, this.result)
    : super(electionId, DateTime.now());
}
...
class ElectionFailed extends ElectionEvent {
  ...
  ElectionFailed(String electionId, this.reason)
    : super(electionId, DateTime.now());
}
...
class CapabilityBasedElectionStrategy implements ElectionStrategy {
  ...
  bool shouldBecomeLeader(
    ...
    for (final candidate in candidates) {
      ...
      if (higherIsBetter
          ? candidatePriority > thisPriority
          : candidatePriority < thisPriority) {
        return false;
      }
    }
