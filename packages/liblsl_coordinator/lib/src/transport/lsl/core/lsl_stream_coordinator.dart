...
abstract class LSLStreamCoordinator implements ResourceManager {
  @override
  ...
  Future<void> start() async {
    _isActive = true;
    await onStart();
    _eventController.add(ResourceStarted(managerId));
  ...
  Future<void> stop() async {
    await onStop();
    _isActive = false;
    _eventController.add(ResourceStopped(managerId));
  ...
  Future<void> dispose() async {
    ...
    final sessionDisposeFutures = _coordinationSessions.values.map((session) async {
      try {
        await session.dispose();
      } catch (e) {
        logger.warning('Error disposing coordination session ${session.managerId}: $e');
      }
    });
  ...
  void addCoordinationSession(CoordinationSessionManager session) {
    ...
    _eventController.add(ResourceAdded(managerId, session.managerId));
  }
  ...
  Future<void> removeCoordinationSession(String sessionId) async {
    ...
    if (session != null) {
      try {
        await session.dispose();
        _eventController.add(ResourceRemoved(managerId, sessionId));
      } catch (e) {
  ...
  void addStreamManager(LSLStreamManager manager) {
    ...
    _eventController.add(ResourceAdded(managerId, manager.resourceId));
  }
  ...
  Future<void> removeStreamManager(String managerId) async {
    ...
    if (manager != null) {
      try {
        await manager.dispose();
        _eventController.add(ResourceRemoved(this.managerId, managerId));
      } catch (e) {
  ...
  List<String> get coordinationSessionIds => _coordinationSessions.keys.toList();
  ...
  void _markResourceErrored(String resourceId, String error) {
    ...
    _eventController.add(ResourceError(managerId, resourceId, error));
    logger.warning('Resource $resourceId marked as errored: $error');
  }
}

/// Interface for coordination session management
abstract class CoordinationSessionManager implements ResourceManager {
  // This will be implemented by LSLCoordinationSession
}

/// Resource events for coordinator
sealed class ResourceEvent {
  final String managerId;
  final DateTime timestamp;
  
  const ResourceEvent(this.managerId, this.timestamp);
}

class ResourceStarted extends ResourceEvent {
  ResourceStarted(String managerId) : super(managerId, DateTime.now());
}

class ResourceStopped extends ResourceEvent {
  ResourceStopped(String managerId) : super(managerId, DateTime.now());
}

class ResourceAdded extends ResourceEvent {
  final String resourceId;
  
  ResourceAdded(String managerId, this.resourceId) : super(managerId, DateTime.now());
}

class ResourceRemoved extends ResourceEvent {
  final String resourceId;
  
  ResourceRemoved(String managerId, this.resourceId) : super(managerId, DateTime.now());
}
...
class ResourceError extends ResourceEvent {
  ...
  ResourceError(String managerId, this.resourceId, this.error) : super(managerId, DateTime.now());
}