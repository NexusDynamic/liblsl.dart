...
class LSLNetworkFactory {
  ...
  ///
  /// Returns a [NetworkSession] that provides the complete end-to-end workflow
  Future<NetworkSession> createNetwork({
    ...
    final eventBus = InMemoryNetworkEventBus();
  ...
  NetworkSession? getNetworkSession(String sessionId) {
    final session = _activeSessions[sessionId];
    if (session == null) return null;
    return NetworkSession._(session, this);
  }
...
class NetworkSession implements universal.NetworkSession {
  final LSLCoordinationSession _session;
  final LSLNetworkFactory _factory;

  NetworkSession._(this._session, this._factory);
