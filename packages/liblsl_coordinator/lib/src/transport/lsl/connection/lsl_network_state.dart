...
class LSLNetworkState implements NetworkState {
  ...
  Future<void> initialize() async {
    ...
    try {
      // Create outlet for publishing our node state
      await _createNodeStateOutlet();

      // Start discovering other nodes
      await _startNodeDiscovery();

      // Start periodic state publishing
      _startPeriodicStatePublishing();
  ...
  /// Start periodic publishing of our node state
  void _startPeriodicStatePublishing() {
    _statePublishTimer = Timer.periodic(const Duration(seconds: 3), (_) async {
      await _publishNodeState();
    });
  }
