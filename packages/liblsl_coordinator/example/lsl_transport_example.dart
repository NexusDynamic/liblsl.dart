...
Future<void> _setupCoordinatorDataConsumption(LSLDataStream dataStream) async {
  ...
  dataStream.inbox.listen((data) {
    ...
    if (DateTime.now().millisecond % 20 == 0) {
      // Approximate 50Hz display
      final summary = data.data
          .take(3)
          .map((v) => v.toStringAsFixed(1))
          .join(', ');
      print(
        '📥 Coordinator received: [$summary...] (${data.data.length} channels)',
      );
    }
  });
...
void _startDataGeneration(LSLDataStream dataStream) {
  ...
  Timer.periodic(Duration(microseconds: 2000), (timer) {
    ...
    if (sampleCount % 2500 == 0) {
      // 2500 samples = 5 seconds at 500Hz
      final seconds = sampleCount / dataStream.sampleRate;
      print(
        '📊 Generated $sampleCount samples (${seconds.toStringAsFixed(1)}s)',
      );
    }
  });

  print(
    '📊 Data generation started (${dataStream.channelCount} channels @ ${dataStream.sampleRate}Hz)',
  );
}
