...
class StatsViewWidget extends StatelessWidget {
  ...
  Map<String, Map<String, dynamic>> _calculateStatistics() {
    ...
    final uniqueSources = csvData['sourceId'].unique();
    final uniqueReporters = csvData['reportingDeviceId'].unique();

    // Any sample recieved, for any device
    ...
    final reportingDeviceId = csvData['reportingDeviceId'].indices(
      sampleRecievedIndices.data,
    );
    ...
    for (final myDeviceId in uniqueReporters) {
      ...
      final myDeviceIndices = csvData['reportingDeviceId'].isEqual(myDeviceId);
      final sampleSentIndices = csvData['event_type'].isEqual(
      ...
      stats['Inter-Sample Interval $myDeviceId'] = {
      ...
      for (final sourceId in uniqueSources) {
        ...
        for (int i = 0; i < recievedCounter.length; i++) {
          ...
          latency[cIndex] = (recievedTimestamps.data[i] as double);
        }

        for (int i = 0; i < sentCounter.length; i++) {
          if (recievedSourceId.data[i] != sourceId) {
            continue;
          }
          ...
          latency[cIndex] -= (timestampColumn.data[i] as double);
        }
        ...
        stats['Latency $sourceId to $myDeviceId'] = {
